...
## Tasks
- [x] K1. Download manager cancel/retry races (drive_download_manager.dart):
...
- [x] K3. Startup robustness (drive_book_repository.dart resetStaleDownloads
...
- [x] K4. Sanitise Drive names used as filesystem paths. Centralise in one
...
- [x] K5. Scanner root guard (scanner_service.dart:61): wrap root listing in
...
- [x] K6. Chapter ordering guarantee: sort chapters ascending by start time at
...
- [x] K7. m4b `meta` FullBox fix (m4b_chapter_parser.dart:97): `meta` carries a
...
- [x] K8. opf namespace tolerance (opf_parser.dart): resolve elements by
...
- [x] K9. Remove dead `enqueueNextFiles` (drive_download_manager.dart:44-62) if
      grep confirms no lib/ callers (mocks reference it — regenerate mocks in
      same commit). Analyzer clean afterwards.
- [x] K10. CHANGELOG entries under [Unreleased] (robustness fixes user-visible
...
- K5: portable failure-injection at Directory level isn't feasible in tests (needs POSIX
  perms or IO seams); shipped as log+rethrow (behaviour preserved for callers, intent
  documented). Classification coverage already exists via library_helpers_test.
- K1: added injectable retryDelay ctor param + queue.retryPending marker so the
  cancel-vs-retry window is deterministic under real timers.
- K9: mocks still reference removed enqueueNextFiles � regenerated in same commit.
